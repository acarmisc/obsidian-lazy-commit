...
| `Error: Formula reports different checksum` from `brew install` | The formula's `sha256` doesn't match the source tarball that GitHub serves for the pinned tag (a local `git archive` and GitHub's archive can produce different shas) | Re-`brew install`; if it persists, update the sha in `Formula/obsidian-lazy-commit.rb` to match `shasum -a 256` of the GitHub tarball (`curl -sL <url>.tar.gz`) and push a new commit. |
